    ...
    <b-card>
      <b-container fluid>
        ...
        <b-table :striped="true"
          ...
          <template #empty>
            <p align="center">Não existe tipo de documentação.</p>
          </template>

          <template #cell(actions)="row">

            <b-button class="border-0 btn-transition btn btn-outline-vivo" size="sm" @click="$router.push( `/documentationRegister/${row.item.id}` )">
        ...
        <b-modal :id="infoModal.id" v-model="showModal" :title="infoModal.title" ok-only @hide="resetInfoModal">
          <template v-slot:modal-footer="{ hide }">
            ...
            <b-button size="sm" variant="outline-vivo" @click="deleta( infoModal.idDocumentacao )">
              Deletar
            </b-button>
          </template>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        heading: 'Tipo Documentação',
        subheading: 'Lista de tipo de documentação.',
        icon: 'pe-7s-drawer icon-gradient bg-happy-itmeo',
        link: '/documentationRegister',

        filter: null,
        filterOn: [],

        fields: [ 
          { key: 'nome', label: 'Documentação', sortable: true, sortDirection: 'desc' },
          { key: 'quantidadeDocumentos', label: 'Documentos', sortable: true, sortDirection: 'desc' },
    ...
    methods: {
      ...
      deleta(id){
        ...
        if(id != undefined){
          ...
          console.log('Documentaçãos que será deletado ' + id);

          api
            .delete(`/tipoDocumentacao/${id}`)
            .then((res) => {
              this.$notify({
                type: 'success',
                title: 'Sucesso',
      ...
      deletaModal(item, button) {
        ...
        this.infoModal.idDocumentacao = item.id
